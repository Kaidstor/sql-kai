...
// pending-INSERT rows interleaved under their duplicate sources. Rows are
// windowed (useRowWindow): only the visible slice plus overscan is mounted,
// two spacer rows preserve the full scroll height. The parent owns the
// scroller ref and scroll handler (header sync + position memory).
import { useMemo, type CSSProperties, type ReactNode, type RefObject, type UIEvent } from "react";
...
export function GridBody({
  ...
  // Flat display list: entry ≥ 0 is a data row index, entry < 0 is a pending
  // insert encoded as -(ii + 1). Inserts render right under their duplicate-
  // source row; ones without a source on this page go to the bottom. Null when
  // there are no inserts — then entry i IS row i and the 50k-element array is
  // skipped entirely.
  ...
  const entries = useMemo(() => {
    ...
    for (let ri = 0; ri < result.rows.length; ri++) {
      list.push(ri);
      const dups = after.get(ri);
      if (dups) for (const ii of dups) list.push(-(ii + 1));
    }
    for (const ii of bottom) list.push(-(ii + 1));
    return list;
  }, [result, inserts]);
  const count = entries ? entries.length : result.rows.length;

  const win = useRowWindow(count, layout.sized, scrollRef, layout.bodyTableRef);

  // The row/insert being edited stays mounted even when scrolled out of the
  // window: unmounting CellInput would silently drop the draft (blur doesn't
  // fire on unmount). An off-window pinned row renders inside the spacer zone
  // at a technically wrong offset — invisible, it's off-screen by definition.
  const entryIndexOf = (e: number) => (entries ? entries.indexOf(e) : e);
  const pinned: number[] = [];
  if (editCell) pinned.push(entryIndexOf(editCell.row));
  if (editIns) pinned.push(entryIndexOf(-(editIns.index + 1)));
  const above = pinned.filter((i) => i >= 0 && i < win.start).sort((a, b) => a - b);
  const below = pinned.filter((i) => i >= win.end).sort((a, b) => a - b);
  const windowIdxs: number[] = [...above];
  for (let i = win.start; i < win.end; i++) windowIdxs.push(i);
  windowIdxs.push(...below);
  const padTop = (win.start - above.length) * win.rowH;
  const padBottom = (count - win.end - below.length) * win.rowH;

  const visibleColCount = result.columns.length - hiddenCols.size + 1;
  const spacer = (h: number, key: string) => (
    <tr key={key} data-spacer style={{ height: h }}>
  ...
  return (
    <div
      ...
      onScroll={(e) => {
        onScroll(e);
        win.onScroll(e.currentTarget.scrollTop);
      }}
    ...
    >
      <table ref={layout.bodyTableRef} style={tableStyle} className={tableClass}>
        {colgroup}
        <tbody>
          {padTop > 0 && spacer(padTop, "top")}
          {windowIdxs.map((i) => {
            const e = entries ? entries[i] : i;
            return e < 0 ? renderInsert(-e - 1) : renderRow(e);
          })}
          {padBottom > 0 && spacer(padBottom, "bottom")}
        </tbody>
      </table>
