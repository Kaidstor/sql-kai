...
- **Синтетические keydown**: обработчики на `window` ловят
  `new KeyboardEvent('keydown', …)` (свой JS не проверяет `isTrusted`),
  а вот встроенная обработка элементов (Esc в нативном диалоге, ввод
  символов) на синтетику не реагирует — диалоги закрывай кликом по
  Cancel/крестику, не синтетическим Esc.
