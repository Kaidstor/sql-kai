...
Кроме таблиц и вьюх дамп показывает RLS-политики, standalone-секвенции, домены,
composite-типы, границы партиций и то, как материализована generated-колонка
(`stored`/`virtual`). Нужен **PostgreSQL 12+**: дамп читает `attgenerated`,
которого раньше не было, и на старом сервере команда честно откажет — там бери
структуру по частям (`tables`, `columns`, `indexes`, `ddl`).
