...
```bash
sql-kai schema <alias> [--json]          # ВСЯ схема одним вызовом — начинай отсюда
...
с колонками, типами, констрейнтами, индексами и триггерами, плюс enum-типы и
функции — за один запрос к каталогу и из одного снимка. Не обходи базу циклом
`tables` → `columns`/`ddl`/`indexes`: это десятки round-trip и рваная картина.
Флаги: `--schema <name>` (сузить), `--definitions` (тела вьюх/функций),
`--comments`, `--internal` (системные схемы и объекты расширений — обычно шум).
...
- **`--write` — только с явного подтверждения пользователя** в текущей сессии.
  ...
  сервиса, а не «вживую».
- **В production-профиль одного `--write` не хватает.** Разрешение даёт человек:
  ввод имени профиля в терминале, флаг `--prod-write` рядом с `--write` или
  `SQL_KAI_ALLOW_PROD_WRITE=<профиль>` в окружении. Получил отказ — не подбирай
  обход и не выставляй эту переменную сам: покажи отказ пользователю и спроси.
  Обкатать миграцию можно на копии: `sql-kai fork <alias>` поднимает базу
  профиля в локальном docker и заводит профиль на неё.
