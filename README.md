...
и CLI `kai` поверх того же ядра. Историческое имя папки — `sql-tauri`.
...
kai holder stop                 # погасить фоновый держатель сессий
```
...
- `kai discover` сам находит postgres-контейнер на хосте (`docker ps` → env
  ...
  `--container <имя> --name <профиль>` (флаг есть и у `kai exec`).
...
- **Сессии между запусками (holder):** запущенный GUI обслуживает `kai q`
  своей сессией через брокер; без GUI первый `kai q` сам поднимает фоновый
  `kai holder run` с тем же протоколом — pg-сессии и ssh-туннели живут между
  ...
  окружения), гаснет сам через ~2 мин без сессий; немедленно — `kai holder
  stop`, vault lock в GUI или `kai vault revoke`. Одиночный вызов мимо
