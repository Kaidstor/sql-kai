...
sql-kai schema <alias> --table t    # то же про одну таблицу: колонки, ключи, индексы, триггеры
...
- **`sql-kai schema <alias>`** — вся структура базы за один поход в каталог
  ...
  `--comments`, `--internal`.
- **`sql-kai schema <alias> --table [schema.]t`** — тот же дамп, суженный до
  одной таблицы: её колонки, констрейнты, индексы, триггеры и политики плюс
  типы, которыми пользуются колонки, функции её триггеров и её секвенции.
  Названную таблицу показывает любой — и листовую партицию, и таблицу
  расширения. Точечные `columns`/`indexes`/`ddl` остаются для `-t`/`--csv`,
  готового `CREATE TABLE` и серверов старше PostgreSQL 12.
...
Tools: **`schema`** (единственный инструмент интроспекции: вся структура базы за
один вызов, либо одна таблица целиком через параметр `table`), `query` (SQL с
`$1..$N`-параметрами или .sql-файлом; чувствительные колонки маскируются),
`tables`, `ddl` (готовый `CREATE TABLE` — единственное, чего нет в дампе),
`open_table`/`open_query` (открыть таблицу или готовый запрос вкладкой в GUI) и
`selection` (что пользователь сейчас видит и выделил — для реплик вроде «эта
строка»). Отдельных `columns`/`indexes` нет намеренно: это подмножества
`schema { table }`, а лишний tool — лишний способ выбрать не тот. У всех
объявлены `annotations` и `outputSchema`, ответ приходит и текстом, и структурой.

Два режима: **закреплённый** (`sql-kai mcp <alias>`) — семь tools по одной базе,
